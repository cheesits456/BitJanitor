...
// Set panel height
document.getElementById("panel-progress").style.height = `${document.getElementById("panel-folder").height}px`
...
function scanDuplicates() {
	...
	walkDuplicates(dir, async function (err, results) {
		if (err) throw err;
		...
		for (const files of Object.values(hashes)) {
			if (files.length > 1) {
				...
				for (const file of files) {
					push.push(`<div id="${file}" class="text-secondary"><span class="mif-bin mif-lg fg-red hover-pointer" onClick="deleteFileDuplicates('${file.replace(/\\/g, "\\\\")}')"></span> ${file}</div>`);
				}
...
function walkDuplicates(dir, done) {
	let results = []
...
function deleteFileDuplicates(file) {
	fs.unlink(file, err => {
